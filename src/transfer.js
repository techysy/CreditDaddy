 ...
 * QoderDaddy 另写入 format/version，并保留旧字段 token 以兼容 v0.1.x 的导入。
 *
 * 能导入：QoderDaddy 明文/加密导出、10router 加密导出（只取 qoder / qoder-cn 账号）、[{name,provider,token}] 数组。
 */
...
export const EXPORT_FORMAT = 'qoderdaddy-accounts';
...
/** 构造导出载荷（10router 兼容字段 + QoderDaddy 旧字段） */
...
export function parseImport(data, { password } = {}) {
  let source = 'qoderdaddy';
  let payload = data;
  if (data?.format === TRANSFER_FORMAT) {
    payload = openTransfer(data, password);
    source = payload?.format === EXPORT_FORMAT ? 'qoderdaddy' : '10router';
  }
  const list = Array.isArray(payload) ? payload : payload?.accounts;
  if (!Array.isArray(list)) {
    throw new TransferError('BAD_FORMAT', '无法识别的文件：应为 QoderDaddy 或 10router 导出的 JSON');
  }
  const fallbackProvider = PROVIDERS.includes(payload?.provider) ? payload.provider : null;

  const accounts = [];
  let skipped = 0;
  for (const item of list) {
    // 显式标注了其他提供商（如 10router 的 cursor 账号）→ 跳过；未标注才用文件级 provider
    const provider = item?.provider ? (PROVIDERS.includes(item.provider) ? item.provider : null) : fallbackProvider;
    ...
    const email = typeof item.email === 'string' && !/^qoder(-cn)?-user-/.test(item.email) ? item.email : null;
    accounts.push({
      provider,
      token,
